...
Mail.App.List.Manager = function(app) {
	...
	this.renderMessages = function() {
		...
		this.app.api.call('getMailList', options, function(msg) {
			// Clear list area...
			listContainer.html('');
			...
			try {
				...
				self.app.generateTimeoutedErrorWithCallback('malm-rendermessages', APP.t('mailErrorWhenTryingGetListOfMessagesTryingAgain'), 10, function() {
					self.renderMessages();
				});

				console.log(['this.renderMessages', e]);

				return false;
			}

			self.pagination.total = parseInt(result.data.total);
			...
			listManager.renderMessages(result.data.list);
	...
	this.markAs = function(msgsno, markas) {
		...
		self.app.api.call('markAs',  {'account': accountManager.account.id, 'mailbox': accountManager.mailbox.id, 'msgsno': msgsno, 'markas': markas}, function(msg) {
			try {
				var result = jQuery.parseJSON(msg);
			}
			catch(e) {
	...
	this.removeMessages = function(ids) {
		...
		self.app.api.call('removeMessages', {'account': accountManager.account.id, 'mailbox': accountManager.mailbox.id, 'msgsno': ids}, function(msg) {
			try {
				var result = jQuery.parseJSON(msg);
			}
			catch(e) {
				alert('Błąd removeMessages()');
				console.log(e);
				return false;
			}
